    	...
    	for(int j=1;j<PivotTable[0].length;j++){
    		...
    		if(j!=this.bold){
	    		for(int i=1;i<PivotTable.length;i++){
	    			if(!PivotTable[i][j].equals("-")  && !PivotTable[i][this.bold].equals("-")){	    				
		    			if(Double.parseDouble(PivotTable[i][this.bold])>Double.parseDouble(PivotTable[i][j])){
		    				countHigher[j-1]++;
		    			}
		    			else if(Double.parseDouble(PivotTable[i][this.bold])<Double.parseDouble(PivotTable[i][j])){
		    				countLower[j-1]++;
		    			}
		    			else countEqual[j-1]++;
	    			}
